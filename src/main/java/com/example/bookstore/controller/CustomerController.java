...
public class CustomerController {
        ...
        public CustomerDTO getById(
                        @Parameter(description = "ID of the customer to retrieve", example = "101") @PathVariable int id) {
                return service.getById(id);
        }

        @Operation(summary = "Get all customers", description = "Fetch a list of all registered customers")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Customers retrieved successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = CustomerDTO.class)))
        })
        @GetMapping
        ...
        public CustomerDTO update(
                        @Parameter(description = "ID of the customer to update", example = "101") @PathVariable int id,
        ...
        @DeleteMapping("/{id}")
        public void delete(
                        @Parameter(description = "ID of the customer to delete", example = "101") @PathVariable int id) {
